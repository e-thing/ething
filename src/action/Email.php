...
class Email extends Action {
	...
	static public function validate(array &$attributes, array $context){
		...
		$attributes = array_merge(
			array( 'to' => array(), 'subject' => "Notification from e-Thing", 'content' => 'no content', 'attachments' => null ),
			$attributes
		);
		...
		foreach(array_keys($attributes) as $key){
			...
			switch($key){
				...
				case 'to':
					...
					$emails = preg_split('/[ ;,]+/', $attributes[$key]);
					foreach($emails as $email){
						if(!filter_var($email, FILTER_VALIDATE_EMAIL))
							throw new \Exception("field '{$key}' : invalid email address '{$email}'");
					}
					
					if(empty($emails))
						throw new \Exception("field '{$key}' : empty");
					
					$attributes[$key] = $emails;
	...
	protected function call(\Ething\Event\Signal $signal){
		...
		$this->ething()->notify($this->subject,$content,$attachments,$this->to);
	}
