...
class ScriptEngine
{
	...
	static public function run(Ething $ething, $script, array $globals = null, $options = array()){
		...
		$cmd .= ' --filename '.$options['scriptName'];
	...
	static public function runFromFile(File $scriptR, $arguments = ''){
		...
		$argv = array();
		$options = array(
			'scriptName' => \basename($scriptR->name())
		);
		
		if($scriptR instanceOf Script){
			$options['apiKey'] = $scriptR->apikey();
		}
