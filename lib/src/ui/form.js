(function(){
			...
			if(typeof this.options.set == 'function'){
				...
				for(var i=0; i<validators.length; i++)
					if(typeof validators[i] == 'function')
						if(validators[i].call(this,v)===false)
							throw 'invalid value';
			}
		}
		catch(e){
	...
	Form.StandardItem.prototype.validate = function(){
		...
		try {
			if(typeof this.options.get == 'function'){
				...
				for(var i=0; i<validators.length; i++)
					if(typeof validators[i] == 'function')
						if(validators[i].call(this,r)===false)
							throw 'invalid value';
			}
		}
		catch(e){
	...
	*/
	...
	Form.Select = function(opt){
		...
		if((typeof opt.value == 'string') && (opt.value in items))
			opt.value = items[opt.value];
		...
		Form.StandardItem.call(this,$.extend(true,{
			...
			onload: function($input){
				// async set
				if($input.val() === null)
					$input.val((opt.value===null || typeof opt.value == 'undefined') ? $("option:first",$input).val() : opt.value);
	...
	**/
	...
	Form.validator.Integer = function(value){
		if(!( (typeof value == 'string' && /^[+-]?[0-9]+$/.test(value)) || (typeof value == 'number' && Number.isInteger(value)) ))
