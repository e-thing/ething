...
(function (global) {
	...
	*/
	...
	function refresh(callback) {
		...
		return EThing.list().done(function(rs) {
			...
			resources.forEach(function(r){
				...
				if(!f){
					var removedId = r.id();
					remove(r);
					EThing.trigger('ething.resource.removed',[removedId]);
				}
			});
	...
	function add(resource) {
		...
		var id = (resource instanceof EThing.Resource) ? resource.id() : resource.id;
		...
		if(index===false){
			...
			resources[index]._fromJson((resource instanceof EThing.Resource) ? resource.json() : resource);
	...
	function remove(resource,noTraversingUp){
		if(Array.isArray(resource)){
	...
	/*EThing.on('ething.file.created ething.table.created ething.device.created ething.app.created', function(evt, resource){
		add(resource);
	});*/
	...
	global.EThing.arbo = {
		load: load,
		refresh: refresh,
		update: function(r){ return add(r);},
