...
(function (global) {
	...
	*/
	...
	function add(rs, nocheck, notrigger) {
		...
		rs.forEach(function(resource){
			if (resource instanceof EThing.Resource) {
				...
				if(!notrigger)
					trigger('resource-add',{resource:resource});
			}
		},this);
		...
		rs.forEach(function(resource){
			for (var i=0; i<resources.length; i++) {
				...
				if (r.id() == resource.id()) {
					...
					if(!notrigger)
						trigger('resource-remove',{resource:resource});
					break;
				}
			}
		},this);
	...
	function trigger(event, data){
		if(handlers[event]){
			var e = $.Event( event, data );
			var h = handlers[event];
			for(var i=0; i<h.length; i++)
				h[i].apply(EThing.arbo,[e]);
		}
	}
...
var dependencies = '//cdn.rawgit.com/swagger-api/swagger-js/v2.1.18/browser/swagger-client.min.js';
