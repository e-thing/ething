...
from .helpers import clusterIdToName, hex2str, crc
import struct
...
class Message(object):
    ...
    def parse(packet):
        ...
        ln = int(datas[4:8], 16)
        if len(datas) != (5+ln)*2:
            raise Exception("invalid packet length mismatch, %d != %d [%s]" % (
                len(datas), (5+ln)*2, packet))
        ...
        for key in self.attr:
            ...
            if isinstance(value, list):
                ...
                if key == "inClusterList" or key == "outClusterList":
                    ...
                    for i in value:
                        f.append('%s (%s)' %
                                 (value[i], clusterIdToName(value[i])))
                    value = f
