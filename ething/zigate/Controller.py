...
class Controller(object):
	...
	def createDevice (self, addr, model):
		...
		if info:
			...
			try:
				...
				self.log.exception("Zigate: unable to create the device %s" % model)
			
		else:
			self.log.warn("Zigate: unable to create the device, unknown model = %s" % model)
	...
	def processMessage (self, packet):
		...
		self.log.debug("Zigate: packet received : %s" % packet.toHumanReadable())
				...
				if responseListener['sqn'] is False:
					...
					if (not responseListener['waitResponse']) or (not success):
						...
						if callable(responseListener['callback']):
							responseListener['callback'](False if success else statusStr, None)
		...
		if addr :
			...
			if not device:
				self.log.warn("Zigate: receive a message from an unknown device, addr: %s" % addr)
		...
		if type=="8010":  # Version
			...
			if packet['clusterId']=="0000" : # General: basic
				...
				if attrId=="0005": # model identifier
					...
					self.log.info("Zigate: ZigateRead - manufacturer : %s" % manufacturer)
					...
					self.log.info("Zigate: ZigateRead - powerSource : %d (%s)" % (powerSource, powerSourceStr))
