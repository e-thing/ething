...
class Zigate(object):
    ...
    def __init__(self, core):
        ...
        self.rpc.register('device.zigate.send', self.controller_send, callback_name = 'callback')
    ...
    def start_controller (self, device):
        ...
        self.log.info("starting Zigate controller '%s' id=%s type=%s" % (device.name, device.id, device.type))
    ...
    def stop_controller (self, device):
        ...
        if device in self.controllers:
            ...
            self.log.info("stopping Zigate controller '%s' id=%s type=%s" % (controller.gateway.name, controller.gateway.id, controller.gateway.type))
            controller.destroy()
    ...
    def controller_send(self, gatewayId, message, waitResponse = None, callback = None):
        ...
        if gatewayId in self.controllers:
            ...
            controller.send(message, callback = callback, waitResponse = waitResponse)
            
        else:
            raise Exception("unknown Zigate instance for device id %s" % gatewayId)
            ...
            for b in chunk:
                ...
                if o == 1:
                    ...
                    try:
                        ...
                        # skip the line
                        self.log.exception("[serial]: unable to handle the message %s" % self._buffer)
                    self._buffer = ''
                elif o == 2:
