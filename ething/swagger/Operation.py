...
class Operation(object):
    ...
    def createRequest (self, data = None, **options):
        ...
        url = '%s://%s%s%s' % (scheme, self.root.host, self.root.basePath, self.path.pathname)
        ...
        for param in self.parameters:
            ...
            hasDefault = isinstance(param, StandardParameter) and param.hasDefault
            value = None
            ...
            if hasValue:
                ...
                if param.isRequired :
                    if hasDefault:
                        ...
                        raise Exception("missing required param: '%s'" % param.name)
                else:
                    continue
        ...
        if formData:
            ...
            if contentType == 'multipart/form-data':
                # multipart form ...
                boundary = ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(10))
                contentType += '; boundary=' + boundary
                body = Operation.generateMultipartFormDataBody(formData, boundary)
