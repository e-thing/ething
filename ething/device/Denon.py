...
class Denon(Device, Switch):
    ...
    def getStatus(self):
        ...
        """
        r = requests.get("http://%s/goform/formMainZone_MainZoneXmlStatus.xml" % (self.host))
    ...
    def sendCmd(self, cmd):
        r = requests.get("http://%s/goform/formiPhoneAppDirect.xml?%s" % (self.host, cmd))
        return r.ok
    ...
    def sendPostCmd(self, cmd, value = None):
        ...
        r = requests.post("http://%s/goform/AppCommand.xml" % (self.host), data = body.encode('utf8'))
