...
class Http(Device):
    ...
    def dynamic_interface(self):
        ...
        def generate_dyn_method(op):
            ...
            def req(self, **kwargs):
                ...
                r = self.make_request(request_['url'], method=request_[
                                      'method'], headers=request_['headers'], body=request_['body'])
            ...
            if op.parameters:
                ...
                for param in reversed(op.parameters):
                    ...
                    method.arg(
                        param.name, required=param.isRequired, **schema)(req)
    ...
    def make_request(self, path, method='get', headers=None, body=None):
        ...
        query = dict_recursive_update(
            {}, parse_qs(new.query), parse_qs(ref.query))
        ...
        if self.isAuthenticate:
            if self.authMode == 'basic':
                auth = requests.auth.HTTPBasicAuth(
                    self.authUser, self.authPassword)
    ...
    def checkSpecification(ething, swagger):
        ...
        r = requests.post(
            "http://online.swagger.io/validator/debug", data=json.dumps(swagger))

        if r.status_code != 200:
            raise Exception(
                'unable to reach http://online.swagger.io/validator/debug')
