...
class MQTT(object):
    ...
    def start_controller (self, device):
        ...
        self.log.info("starting MQTT controller '%s' id=%s type=%s" % (device.name, device.id, device.type))
    ...
    def stop_controller (self, device):
        ...
        if device in self.controllers:
            ...
            self.log.info("stopping MQTT controller '%s' id=%s type=%s" % (controller.device.name, controller.device.id, controller.device.type))
            controller.destroy()
    ...
    def controller_send(self, device_id, topic, payload):
        ...
        if device_id in self.controllers:
            ...
            raise Exception("unknown MQTT instance for device id %s" % device_id)
