...
from .MySensorsSensor import MySensorsMsgReceived
...
class MySensorsProtocol(LineReader):
    ...
    def createNode(self, nodeId):
        ...
        LOGGER.info("new node nodeId=%d" % nodeId)
    ...
    def createSensor(self, node, sensorId, sensorType, name=None):
        ...
        if not sensor:
            raise Exception("fail to create the sensor nodeId=%d sensorId=%d sensorType=%s[%s]" % (
                node.nodeId, sensorId, sensorType, sensorTypeStr(sensorType)))

        LOGGER.info("new sensor nodeId=%d sensorId=%d sensorType=%s[%s]" % (
            node.nodeId, sensorId, sensorType, sensorTypeStr(sensorType)))
        ...
        try:
            ...
            with self.plugin as plugin:
                ...
                with node, sensor:
                        ...
                        try:
                            ...
                            if message.messageType == PRESENTATION:
                                ...
                                if sensor:
                                    ...
                                    LOGGER.debug("set value nodeId=%d sensorId=%d valueType=%d value=%s",
                                    ...
                                    if datatype:
                                        ...
                                        LOGGER.warning(
                                            "unknown value subtype %d" % message.subType)
                                    ...
                                    if datatype:
                                        ...
                                        if value is not None:
                                            ...
                                            if payload is not None:
                                                response = Message(nodeId, sensorId, SET, message.subType,
                                                                   payload=payload)
                                ...
                                if message.subType == I_GATEWAY_READY:
                                    ...
                                    LOGGER.info(
                                        "gateway version = %s" % self.gatewayLibVersion)
                                    ...
                                    LOGGER.info("log: nodeId=%d sensorId=%d %s" % (
                                    ...
                                    LOGGER.warning(
                                        "message not processed : %s" % str(message))
                    ...
                    if sensor:
                        if sensor.debug:
                            sensor.store('debug_msg', message.to_dict())
                        sensor.emit(MySensorsMsgReceived(sensor, message))
