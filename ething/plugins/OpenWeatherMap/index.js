(function(global){
    ...
    */
    ...
    var WDeviceMultiLabel = widget.find('WDeviceMultiLabel')

    widget.register({
        name: 'WOpenWeatherMapDevice',
        template: '<w-device-multi-label :items="items" v-bind="$attrs" @error="$emit(\'error\', $event)"/>',
        components: {
            WDeviceMultiLabel
        },
    ...
    function getWeatherForecast (appId, location, done) {
        var requestUrl = OPEN_WEATHER_MAP_URL + '&q=' + encodeURIComponent(location) + '&appid=' + encodeURIComponent(appId)
