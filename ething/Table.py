...
@attr('maxLength', validator = isNone() | isInteger(min=1), default = 5000)
@attr('expireAfter', validator = isNone() | isInteger(min=1), default = None)
@attr('length', default = 0, mode = READ_ONLY)
@attr('keys', default = {}, mode = READ_ONLY)
	...
	def insert (self, data, invalidFields = INVALID_FIELD_RENAME):
		if data:
			...
			if maxLength and length > maxLength :
				...
				removedDoc = c.find_one_and_delete({}, sort={'date':1})
