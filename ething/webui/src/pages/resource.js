(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        define(['ui/core', 'text!./resource.html', 'jquery', 'ething', 'form', 'css!./resource'], factory);
    }
	...
	return function(data){
		...
		$.each(UI.meta.getResourceProperties(resource), function(name, property){
		...
		if(Object.keys(categories).length > 1){
			...
			Object.keys(categories).forEach(function(cat){
				...
				catForms.push(UI.meta.getResourceForm(resource, categories[cat]).then(function(formItems){
					
					var item;
					
					// special case
					if( (resource instanceof EThing.Device) && resource.isTypeof('Http') && /server/i.test(cat) ){
						item = new $.Form.FieldsEnabler({
							label: 'Enable',
							item: new $.Form.FormLayout({items: formItems}),
							state: !!resource.attr('url'),
							disabledValue: {
								url: null
							}
						});
			...
			form = UI.meta.getResourceForm(resource).then(function(formItems){
