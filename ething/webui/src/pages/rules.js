(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        define(['ui/core', 'text!./rules.html', 'jquery', 'ething', 'cronstrue', 'css!./rules',
			'ui/formmodal',
			'ui/resourceselect',
			'ui/http',
			'ui/devicerequest',
	...
	
	{
		...
		category: 'device',
		toString: function(options){
			return ResourceSelection.toString(options.resource, 'device') + ' has new data';
		},
		description: 'Is fired each time a device has new data.',
		form: function(){
			return new $.Form.FormLayout({
				items: [{
					name: 'resource',
	...
	var editRule = function(rule, callback){
		...
		var editItem = function(type, item, callback){
			...
			Rule[type+'s'].forEach(function(meta){
				...
				var form = function(){ // delay the creation of the form, save cpu usage
					return typeof meta.form == 'function' ? (meta.form(meta) || null) : null;
				};
