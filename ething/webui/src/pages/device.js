(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        define(['require', 'ui/core', 'jquery', 'ething', 'marked', 'form', 'highlight', 'css!./device', 'ui/browser'], factory);
    }
}(this, function (require, UI, $, EThing, marked) {
	...
	OperationView.prototype.execute = function( parameters, success, error){
		...
		var contentType = this.operationApi['return'],
			self = this;
	...
	OperationView.prototype.toUrl = function( parameters ){
		...
		if(parameters && !$.isEmptyObject(parameters)){
			url += '?'+ EThing.utils.param(parameters);
		}
	...
	var Infopanel = function($element, device){
		...
		props = UI.meta.get(device, 'properties');
		for(var name in props){
			if(['description', 'data', 'name'].indexOf(name) !== -1)
				props[name] = null;
			else {
				props[name] = props[name].formattedValue()
			}
		}
			...
			draw: function(){
				...
				require(['plot'], function(){
					...
					var tables = EThing.arbo.find(function(r){
						return r instanceof EThing.Table && r.createdBy() === device.id();
					});
			...
			draw: function(){
				...
				device.getApi().done(function(api){
					...
					operations.forEach(function(operation){
						...
						if(operation['return']) $operation.find('.operation-header-mime').text(operation['return']);
						...
						// special MySensors
						if(name === 'sendMessage' && form && /^MySensors/.test(device.type())){
		...
		return $.extend({}, contentpanelItem, {
			...
			draw: function(){
				...
				var sensors = EThing.arbo.find(function(r){
					return r instanceof EThing.Device && r.createdBy() === device.id();
				});
				
				sensors.forEach(function(sensor){
					
					var factory = UI.widgetManager.resource(sensor);
					
					if(factory){
						try {
							var widget = factory.instanciate();
	...
	var WidgetContentpanelItem = function(device){
		...
		return $.extend({}, contentpanelItem, {
			...
			draw: function(){
				...
				var factory = UI.widgetManager.resource(device);
	...
	var ParentContentpanelItem = function(device){
		...
		return $.extend({}, contentpanelItem, {
			...
			draw: function(){
				...
				var c = device;
				while(c && c.createdBy()){
					var p = EThing.arbo.findOneById(c.createdBy());
					if(p) parents.push(p);
					c = p;
				}
	...
	var InclusionContentpanelItem = function(device){
		...
		return $.extend({}, contentpanelItem, {
			...
			draw: function(){
				...
				function isInclusionEnabled(){
					return !!device.data('inclusion', false);
				}
				function setInclusionState(state){
					$inclusionButton.text(state ? 'Stop inclusion' : 'Start inclusion');
				}
				function updateInclusionState(){
					setInclusionState(isInclusionEnabled());
				}
				
				updateInclusionState();
				
				$inclusionButton.click(function(){
					var state = isInclusionEnabled();
					device.execute(state ? 'stopInclusion' : 'startInclusion').done(function(){
						setInclusionState(!state);
					});
				});
				
				this.onUpdateResource = function(evt,updatedKeys){
					if(typeof updatedKeys !== 'undefined' && updatedKeys.indexOf('data')===-1) return;
					updateInclusionState();
				};
				
				device.on('updated', this.onUpdateResource);
			...
			destroy: function(){
				contentpanelItem.destroy.call(this);
				device.off('updated', this.onUpdateResource);
			}
	...
	WidgetContentpanelItem,
	{
		...
		filter: function(r){
			return r.methods().indexOf('startInclusion') !== -1 && r.methods().indexOf('stopInclusion') !== -1;
		}
	},
	{
		...
		filter: function(d){
			return d.isTypeof("RFLinkNode") && d.attr('subType') === 'switch';
		}
	}, {
		class: SensorsContentpanelItem,
		filter: function(d){
			return EThing.arbo.find(function(r){
				return (r instanceof EThing.Device) && r.createdBy() === d.id();
			}).length;
		}
	},{
		...
		filter: function(d){
			return EThing.arbo.find(function(r){
				return r instanceof EThing.Table && r.createdBy() === d.id();
			}).length;
		}
	},{
