			...
			return $.extend({}, widget, {
				...
				draw: function(){
					...
					this.$element.find('svg.state').css({
						...
						self.isOn() ? self.turnOff() : self.turnOn();
					});
					
					this.resourceUpdateFn = function(){
						self.updateState();
					};
					
					sensor.on('DeviceDataSet', this.resourceUpdateFn);
				...
				turnOn: function(){
					var self = this;
					return self.async(sensor.setState(true).done(function(){
						self.setState(true);
					}));
				},
				...
				turnOff: function(){
					var self = this;
					return self.async(sensor.setState(false).done(function(){
						self.setState(false);
					}));
				},
				...
				async: function(dfr){
					...
					dfr.done(function(){
						...
						var $error = self.$element.find('.error');
						$loader.hide();
						$error.show();
						setTimeout(function(){
							$error.hide();
				...
				updateState: function(){
					...
					sensor.getState().done(function(state){
						self.setState(state);
					});
