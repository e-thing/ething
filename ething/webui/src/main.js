(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
		...
		requirejs.config({
			...
			paths: {
				...
				'ething' : '../../node_modules/ething-js/dist/ething',
        ...
        define(['require', 'ething', 'jquery', 'ui/core'], factory);
    }
	...
	*/
	...
	function auth(){
		if(!auth_disabled){
			window.location.replace(sessionUrl+'/login?redirect_uri='+encodeURIComponent(window.location.href));
		}
	}
	...
	if(csrf_token || auth_disabled){
		...
		console.error('no Csrf-token cookie found !');
		auth();
	}
