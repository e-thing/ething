        ...
        if event_type in self.handlers:
            for handler in self.handlers[event_type]:
                try:
                    ...
                    self.log.exception("Error calling signal handler with signal: %s handler: %s" % (
                        str(signal), handler))
    ...
    def dispatch(self, signal):
        ...
        for dispatcher in self.dispatchers:
            try:
                ...
                self.log.exception("Error calling signal dispatcher with signal: %s dispatcher: %s" % (
                    str(signal), dispatcher))
