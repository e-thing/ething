                    ...
                    try:
                        ...
                        if message.messageType == PRESENTATION:
                            ...
                            if message.subType == I_GATEWAY_READY:
                                ...
                                response = Message(
                                    message.nodeId, message.childSensorId, INTERNAL, NO_ACK, I_CONFIG, 'M' if gateway.isMetric else 'I')
                                self.send(response)
                            ...
                            if message.subType == ST_FIRMWARE_CONFIG_REQUEST:
                                ...
                                if len(parts) >= 4:
                                    ...
                                    self.log.info("MySensors: FW CONFIG : nodeId=%d type=%04X version=%04X blocks=%d crc=%04X BLVersion=%04X" % (
                                    ...
                                    if nodeId in self._pendingFirmware:
                                        ...
                                        ok = firmwareInfo['type'] == type and firmwareInfo['version'] == version and firmwareInfo[
                                            'blocks'] == nbBlocks and firmwareInfo['crc'] == crc

                                        if ok:
                                            self.log.info("MySensors: FW updated successfully : nodeId=%d type=%04X version=%04X blocks=%d crc=%04X" % (
                                ...
                                if len(message.payload) >= 6:
                                    ...
                                    if nodeId in self._pendingFirmware:
                                        ...
                                        chunk = self._pendingFirmware[nodeId]['firmware'][iBlock*Controller.FIRMWARE_BLOCK_SIZE:(
                                            iBlock+1)*Controller.FIRMWARE_BLOCK_SIZE]
                                        response = Message(message.nodeId, INTERNAL_CHILD, STREAM, NO_ACK, ST_FIRMWARE_CONFIG_RESPONSE, pack(
                                        ...
                                        if stream['type'] == message.subType:
                                            ...
                                            if index == 0 or expectedIndex == index:
                                                ...
                                                if index == 0:
                                                    ...
                                                    self.log.warn("MySensors: STREAM: end of stream, packetCount=%d , size(B)=%d , time(s)=%f" % (
