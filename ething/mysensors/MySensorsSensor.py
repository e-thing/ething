...
class isSensorType(Validator):
    def validate(self, value, object):
        ...
        if formatted is None:
            raise ValueError(
                'must be a valid integer or a string describing a sensor type')
        return formatted
...
class MySensorsSensor (Device):
    ...
    def dynamic_interface(self):
        ...
        if sensorType in type_to_interface:
            interface.inherit(self, type_to_interface[sensorType], filter=lambda func: (
                'optional' not in func.meta) or self.hasData(valueTypeToName(func.meta['optional'])))
