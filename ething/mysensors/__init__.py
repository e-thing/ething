...
class MySensors(object):
    ...
    def __init__(self, core):
        ...
        self.rpc.register('device.mysensors.send', self.controller_send, callback_name = 'callback')
        self.rpc.register('device.mysensors.sendWaitResponse', self.controller_send_wait_response, callback_name = 'callback')
        self.rpc.register('device.mysensors.updateFirmware', self.controller_update_firmware, callback_name = 'callback')
    ...
    def start_controller (self, device):
        ...
        self.log.info("starting MySensors controller '%s' id=%s type=%s" % (device.name, device.id, device.type))
    ...
    def stop_controller (self, device):
        ...
        if device in self.controllers:
            ...
            self.log.info("stopping MySensors controller '%s' id=%s type=%s" % (controller.gateway.name, controller.gateway.id, controller.gateway.type))
            controller.destroy()
    ...
    def controller_send(self, gatewayId, message, callback):
        ...
        if gatewayId in self.controllers:
            ...
            raise Exception("unknown MySensors instance for device id %s" % gatewayId)
        ...
        if node:
            ...
            if gateway:
                ...
                if gateway.id in self.controllers:
                    ...
                    raise Exception("unknown MySensors instance for device id %s" % gateway.id)
            else:
                raise Exception("unable to find gateway for node %s" % str(node))
        else:
...
class EthernetTransport(object):
    ...
    def open(self):
        ...
        self.log.info("[Ethernet]: connection opened, host: %s , port: %d" % (o.hostname, o.port))
