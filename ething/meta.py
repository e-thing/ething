...
class InterfaceDecorator(object):
    ...
    def inherit(self, cls, base=None, filter=None):
        ...
        if is_instance:
            # dynamic inheritance !
            ii = getattr(cls, '__interfaces', []) + \
                [c for c in base.__mro__ if getattr(c, 'meta', None)]
...
class MethodDecorator(object):
    ...
    def bind_to(self, instance):
        def d(func):
            ...
            object.__setattr__(instance, func.meta['name'], func.__get__(
        ...
        for arg_name in kwargs:
            ...
            if 'type' in arg_meta:
                ...
                if arg_meta['type'] not in read_types:
                    raise ValueError("%s(): argument '%s' has wrong type, got %s instead of %s" % (
                        self.name, arg_name, type(v).__name__, arg_meta['type']))
...
class Interface(object):
    ...
    def build(self):
        ...
        self.__bases = [c for c in self.device.__class__.__mro__ if getattr(
            c, 'meta', None)] + getattr(self.device, '__interfaces', [])
